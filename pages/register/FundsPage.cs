...
namespace zCustodiaUi.pages.register
{
    public class FundsPage
    {
        ...
        public async Task RegisterNewFundNegative()
        {
            var tomorrow = DateTime.Now.AddDays(1).Day.ToString();
            var today = DateTime.Now.Day.ToString();
            ...
            // Fill mandatory additional fields
            await util.ScrollToElementAndMaintainPosition(gen.LocatorMatLabel("Prazo de Recepção da Chave da NFe"), "Scroll to NFe Key Receipt Deadline");
            await util.Write(gen.LocatorMatLabel("Prazo de Recepção da Chave da NFe"), "3", "Write NFe Key Receipt Deadline");

            await util.Write(gen.LocatorMatLabel("Prazo Recepção do Lastro"), "3", "Write Deadline Reception Ballast");

            await util.ScrollToElementAndMaintainPosition(gen.LocatorMatLabel("Perfil Liquidação Sistema de Ativos"), "Scroll to Profile Active System and maintain position");

            await util.Write(gen.LocatorMatLabel("Prazo PDD"), "0", "Write Deadline PDD");
            await util.Write(gen.LocatorMatLabel("Num. Sequencial Att Termo Cessão"), "0", "Write Sequence Number Term Cession");
            await util.Write(gen.LocatorMatLabel("Num. Sequencial Att Termo Recompra"), "0", "Write Sequence Number Term Repurchase");
            //await util.Write(gen.LocatorMatLabel("Qtd. Dias Retroagir Importação PL"), "10", "Write Quantity Days Import PL");
            //await util.Write(gen.LocatorMatLabel("Convênio"), "12345", "Write Agreement");
            await util.Write(gen.LocatorMatLabel("Valor Máximo para o Robô Assinatura"), "0", "Write Max Value To Assign Robot");
            ...
            await util.Write(gen.LocatorMatLabel("Código Carteira"), "0", "Write Wallet Code");
            await util.Write(gen.LocatorMatLabel("Ordem de Processamento"), "99999", "Write Process Order");
            ...
            await util.Click(el.AssignRadio(false), "Select 'yes' to assign");
            await util.Click(el.SignsByEndorsementRadio(false), "Select 'yes' to Signs By Endorsement");
            await util.Click(el.AssignTermRadio(false), "Select 'yes' to assign Term");
            await util.Click(el.IssuesDuplicateRadio(true), "Select 'yes' to Issue Duplicate");
