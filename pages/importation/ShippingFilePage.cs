...
namespace zCustodiaUi.pages.importation
{
    public class ShippingFilePage
    {
        ...
        public async Task ValidateFilePresentOnTable()
        {
            ...
            var getId = await _util.ValidateIfElementHaveValue(_el.IdPositionOnTheTable(nameNewFile), "Validate if file name is correct in the grid");
        ...
        {
            await _util.ValidateTextIsNotVisibleOnScreen(nameNewFile, "Validate if file was don´t be present on table to validate if file was deleted on table");
        }
